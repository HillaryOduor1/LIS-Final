    ...
    <nav className="fixed inset-x-0 top-0 z-50 bg-white/90 dark:bg-background-dark/90 backdrop-blur-md border-b border-primary/10 navbar-fallback">
      <div className="mx-auto max-w-7xl h-16 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link to="/" className="flex items-center gap-3" onClick={function() { trackEvent('logo_click'); triggerHaptic(); }}>
            ...
            if (link.href.indexOf("#") !== -1) {
              return (
                <a
                  key={link.name}
                  href={link.href}
                  onClick={function(e) { scrollToSection(e, link.href); trackEvent('nav_click', { link: link.name }); triggerHaptic(); }}
              ...
              return (
                <Link
                  key={link.name}
                  to={link.href}
                  onClick={function() { trackEvent('nav_click', { link: link.name }); triggerHaptic(); }}
                  className={"text-sm font-semibold hover:text-primary transition-colors focus:outline-none focus:ring-2 focus:ring-primary rounded px-2 py-1 " + (isActive(link.href) ? 'text-primary' : 'text-[#0d1b14] dark:text-white')}
                >
                  {link.name}
                </Link>
        ...
        <div className="flex items-center justify-end min-w-[44px] theme-toggle-wrapper">
          <ThemeToggle />
        </div>
      </div>
    </nav>

      ...
      <div className="mx-auto max-w-7xl h-16 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link to="/" className="flex items-center gap-3" onClick={() => trackEvent('logo_click')}>
            ...
            <h1 className="text-xl font-extrabold tracking-tight text-[#0d1b14] dark:text-white">Landscapes Integrity SolutionsS</h1>
          </Link>
        </div>
...
export default function Navbar({ isSidebarOpen, setIsSidebarOpen }: NavbarProps) {
  const { content } = useContent();
  const location = useLocation();
  // Safe navigation: content may be null
  const navlinks: NavLink[] = (content && content.navigation) ? (content.navigation as NavLink[]) : [];
  ...
  // Keyboard shortcut: '/' to focus search (if search exists)
  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === '/' && document.activeElement !== document.body) {
        e.preventDefault();
        // Focus on search input – to be implemented in search component
    ...
    <nav className="fixed inset-x-0 top-0 z-50 bg-white/90 dark:bg-background-dark/90 backdrop-blur-md border-b border-primary/10">
      <div className="mx-auto max-w-7xl h-16 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <ThemeToggle />
          <button 
            className="hidden md:block bg-primary hover:bg-primary/90 text-[#0d1b14] px-6 py-2 rounded-lg font-bold text-sm transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
            onClick={() => trackEvent('contact_button_click')}
          >
            Contact Us
          </button>
          <button 
            className="md:hidden bg-primary hover:bg-primary/90 text-[#0d1b14] px-4 py-2 rounded-lg font-bold text-sm transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-primary"
            onClick={() => trackEvent('contact_button_click_mobile')}
          >
            Contact
          </button>
          <Link to="/login" onClick={() => trackEvent('click_login')} className="focus:outline-none focus:ring-2 focus:ring-primary rounded-full">

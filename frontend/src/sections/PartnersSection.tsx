...
const PartnersSection = () => {
  ...
  // Map generic icon names to meaningful material symbols
  const getIconName = (icon: string) => {
    if (icon === 'corporate') return 'apartment';
    if (icon === 'forest') return 'forest';
    if (icon === 'ecology') return 'eco';
    return icon;
  };
          ...
          <div className="lg:col-span-1">
            ...
            <div className="mt-8 space-y-4">
              {categories.map((item, index) => (
                <div key={index} className="flex items-center gap-3 font-semibold text-sm"><span className="material-symbols-outlined text-primary" aria-hidden="true">check_circle</span>{item}</div>
              ))}
            </div>
          </div>
          <div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 opacity-70 grayscale hover:grayscale-0 transition-all">
            {logos.map((partner, index) => (
              <div key={index} className="aspect-square bg-slate-50 dark:bg-slate-800 rounded-lg flex items-center justify-center p-6 border border-slate-100 dark:border-slate-700 hover:shadow-lg transition-all">
                <span className="material-symbols-outlined text-4xl text-slate-400">{getIconName(partner.icon)}</span>
              </div>
            ))}
          </div>
        </div>

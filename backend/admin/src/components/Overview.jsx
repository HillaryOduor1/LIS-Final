...
function ActivityFeed({ navigate }) {
    ...
    const fetchActivity = () => {
        ...
        fetch(`${API_BASE}/activity`)
            .then(res => res.json())
            .then(data => {
                setActivity(data.slice(0, 6)); // latest 6
                setLoading(false);
            })
            .catch(() => setLoading(false));
    };
...
/* ================= QUICK ACTIONS ================= /
const quickActions = [
    { label: 'Edit Content', section: 'content', icon: FileText },
...
function QuickActions({ navigate }) {
    return (
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 border">
            <h3 className="mb-4 font-semibold">Quick Actions</h3>
            <div className="grid grid-cols-2 gap-2">
                {quickActions.map((a, i) => {
                    const Icon = a.icon;
                    return (
                        <button key={i} onClick={() => navigate(a.section)}>
...
import {
    BarChart2, Globe, Users, Server, ArrowUpRight, ArrowDownRight,
    Activity, Settings2, FileText, LogIn, RefreshCw
} from 'lucide-react';
...
var MOCK_ACTIVITY = [
    { id: 1, action: 'settings_save', label: 'Settings updated', detail: 'Appearance — colors changed', user: 'admin', timestamp: new Date(Date.now() - 2 * 60 * 1000).toISOString() },
    ...
    { id: 4, action: 'content_update', label: 'Pricing updated', detail: 'Pro plan price changed', user: 'editor', timestamp: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString() },
];
    ...
    return (
        <div className="card p-5">
            <div className="flex items-center justify-between mb-4">
                ...
                <button
                    onClick={function () { navigate('activity'); }}
            ...
            <div className="space-y-3">
                {activity.map(function (a) {
                    var Icon = ACTION_ICONS[a.action] || ACTION_ICONS.default;
                    return (
                        <div key={a.id} className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="text-xs text-muted truncate">{a.detail} · {timeAgo(a.timestamp)}</div>
                            </div>
                        </div>
                    );
                })}
...
var quickActions = [
    { label: 'Edit Hero', section: 'content', icon: FileText },
    { label: 'Theme Colors', section: 'settings', icon: Settings2 },
    { label: 'Manage Users', section: 'users', icon: Users },
    { label: 'Activity Log', section: 'activity', icon: Activity },
];
            ...
            <div className="grid grid-cols-2 gap-2">
                {quickActions.map(function (a, i) {
                    ...
                    return (
                        <button
                            key={i}
                            onClick={function () { navigate(a.section); }}
                            className="flex items-center gap-2 p-3 rounded-xl text-sm font-medium text-muted hover:bg-surface hover:text-foreground border border-border transition-all text-left"
                        >
                            <Icon size={16} />
                            {a.label}
                        </button>
...
export default function Overview({ navigate }) {
    ...
    var cards = [
        { label: 'Content Sections', value: stats ? String(stats.sectionCount || 7) : '7', icon: BarChart2, trend: '+12%', up: true },
        { label: 'Theme Mode', value: stats ? (stats.themeMode || 'Light') : '—', icon: Globe, trend: null, up: true },
        { label: 'Uptime (s)', value: stats ? String(stats.uptime || 0) : '—', icon: Server, trend: null, up: true },
        { label: 'Registered Users', value: '3', icon: Users, trend: '+1', up: true },
    ];
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div>
                ...
                <p className="text-sm text-muted mt-0.5">Real-time metrics and system status</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {cards.map(function (c, i) { return <StatCard key={i} {...c} />; })}
                </div>
            )}

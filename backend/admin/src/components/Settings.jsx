        ...
        const loadSettings = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    setError('Using default settings - server unavailable');
                }
            } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                localStorage.setItem('cms-settings-backup', JSON.stringify(toSave));
            } else {
                setError('Failed to save settings to server');
            }
        } catch (error) {
            console.error('Failed to save settings:', error);
            setError('Network error - settings saved locally only');
...
var DEFAULT_SETTINGS = {
    ...
    site: {
        title: 'Modern CMS',
        description: 'A powerful admin dashboard.',
    },
    lastUpdated: new Date().toISOString(),
};
...
function Select({ value, onChange, options }) {
    return (
        <select
            ...
            className="input-base px-3 py-1.5 text-sm rounded-lg cursor-pointer"
        >
...
function AppearanceTab({ settings, onChange }) {
    var theme = settings.theme || DEFAULT_SETTINGS.theme;
    ...
    var radiusOptions = [
        { id: 'small', label: 'Sharp', preview: '0.25rem' },
        { id: 'medium', label: 'Rounded', preview: '0.5rem' },
        { id: 'large', label: 'Pill', preview: '1rem' },
    ];

    return (
        <div className="space-y-8 animate-fade-in">
            <SectionHeader title="Theme Mode" description="Select the primary interface color scheme" />
            <div className="grid grid-cols-3 gap-3 mb-8">
                {themeOptions.map(function (opt) {
                    var Icon = opt.icon;
                    var isActive = theme.mode === opt.id;
                    return (
                        <button
                            key={opt.id}
                            onClick={function () { onChange('theme', { ...theme, mode: opt.id }); }}
                            className={[
                                'flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all duration-200 text-sm font-medium',
                                isActive
                                    ? 'border-accent bg-accent/5 text-accent'
                                    : 'border-border text-muted hover:border-muted hover:bg-surface',
                            ].join(' ')}
            ...
            <SectionHeader title="Color Palette" description="Choose brand colors for the frontend" />
            <div className="flex flex-wrap gap-2 mb-6">
                {colorPresets.map(function (p) {
                    var isActive = theme.primaryColor === p.primary;
                    return (
                        <button
                            key={p.name}
                            onClick={function () { onChange('theme', { ...theme, primaryColor: p.primary, secondaryColor: p.secondary }); }}
                            title={p.name}
                            className={[
                                'flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-medium transition-all',
                                isActive ? 'border-accent bg-accent/5' : 'border-border hover:bg-surface',
                        ...
                        >
                            <span className="w-3.5 h-3.5 rounded-full inline-block" style={{ background: p.primary }} />
                            {p.name}
                        </button>
                    );
                })}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="card p-4">
                    <label className="text-xs font-bold text-muted uppercase tracking-wider mb-3 block">Primary Color</label>
                    <div className="flex items-center gap-3">
                        <input
                            type="color"
                            value={theme.primaryColor}
                            onChange={function (e) { onChange('theme', { ...theme, primaryColor: e.target.value }); }}
                            className="w-10 h-10 rounded-lg cursor-pointer border border-border"
                        />
                        <input
                            type="text"
                ...
                <div className="card p-4">
                    <label className="text-xs font-bold text-muted uppercase tracking-wider mb-3 block">Secondary Color</label>
                    <div className="flex items-center gap-3">
                        <input
                            type="color"
                            value={theme.secondaryColor}
                            onChange={function (e) { onChange('theme', { ...theme, secondaryColor: e.target.value }); }}
                            className="w-10 h-10 rounded-lg cursor-pointer border border-border"
                        />
                        <input
                            type="text"
            ...
            <SectionHeader title="Shape & Visual" description="Border radius and visual effects" />
            <div className="grid grid-cols-3 gap-3 mb-6">
                {radiusOptions.map(function (opt) {
                    var isActive = theme.borderRadius === opt.id;
                    return (
                        <button
                            key={opt.id}
                            onClick={function () { onChange('theme', { ...theme, borderRadius: opt.id }); }}
                            className={[
                                'flex flex-col items-center gap-2 p-4 border-2 transition-all text-sm font-medium',
                                isActive ? 'border-accent bg-accent/5 text-accent' : 'border-border text-muted hover:bg-surface',
                            ].join(' ')}
                            style={{ borderRadius: opt.preview }}
                        >
                            <div className="w-8 h-8 border-2 border-current" style={{ borderRadius: opt.preview }} />
                            {opt.label}
                        </button>
                    );
                })}
            ...
            <div className="card p-4 space-y-0">
                <Row label="Drop Shadows" description="Layered shadows for depth and elevation">
                    <Toggle id="shadows" checked={theme.shadows} onChange={function (v) { onChange('theme', { ...theme, shadows: v }); }} />
                </Row>
                <Row label="Animations" description="Smooth UI transitions and micro-interactions">
                    <Toggle id="animations" checked={theme.animations} onChange={function (v) { onChange('theme', { ...theme, animations: v }); }} />
                </Row>
            </div>
        </div>
    );
...
function TypographyTab({ settings, onChange }) {
    var typo = settings.typography || DEFAULT_SETTINGS.typography;

    return (
        <div className="space-y-6 animate-fade-in">
            <SectionHeader title="Typography" description="Font family, size, and text rendering" />
            <div className="card p-4 space-y-0">
                <Row label="Font Family" description="Global typeface for the site">
                    <Select
                        value={typo.fontFamily}
                        onChange={function (v) { onChange('typography', { ...typo, fontFamily: v }); }}
                        options={[
                            { value: 'system', label: 'System Default' },
                            { value: 'serif', label: 'Serif (Georgia)' },
                            { value: 'monospace', label: 'Monospace' },
                ...
                <Row label="Font Size" description="Base font size used across the site">
                    <Select
                        value={typo.fontSize}
                        onChange={function (v) { onChange('typography', { ...typo, fontSize: v }); }}
                        options={[
                            { value: 'small', label: 'Small (14px)' },
                            { value: 'normal', label: 'Normal (16px)' },
                            { value: 'large', label: 'Large (18px)' },
                ...
                <Row label="Letter Spacing" description="Space between individual characters">
                    <Select
                        value={typo.letterSpacing}
                        onChange={function (v) { onChange('typography', { ...typo, letterSpacing: v }); }}
                        options={[
                ...
                <Row label="Text Alignment" description="Default text alignment">
                    <Select
                        value={typo.textAlign}
                        onChange={function (v) { onChange('typography', { ...typo, textAlign: v }); }}
                        options={[
                            { value: 'left', label: 'Left' },
                            { value: 'center', label: 'Center' },
                            { value: 'right', label: 'Right' },
                ...
                <Row label="Body Weight" description="Font weight for body text">
                    <Select
                        value={typo.bodyWeight}
                        onChange={function (v) { onChange('typography', { ...typo, bodyWeight: v }); }}
                        options={[
                            { value: 'normal', label: 'Regular (400)' },
                            { value: 'medium', label: 'Medium (500)' },
                            { value: 'semibold', label: 'Semibold (600)' },
                ...
                <Row label="Heading Weight" description="Font weight for headings">
                    <Select
                        value={typo.headingWeight}
                        onChange={function (v) { onChange('typography', { ...typo, headingWeight: v }); }}
                        options={[
                            { value: 'normal', label: 'Regular (400)' },
                            { value: 'semibold', label: 'Semibold (600)' },
                            { value: 'bold', label: 'Bold (700)' },
...
function UITab({ settings, onChange }) {
    var ui = settings.ui || DEFAULT_SETTINGS.ui;

    return (
        <div className="space-y-6 animate-fade-in">
            <SectionHeader title="UI & Layout" description="Density, button styles, and spacing" />
            <div className="card p-4 space-y-0">
                <Row label="UI Density" description="Spacing and sizing of interface elements">
                    <Select
                        value={ui.density}
                        onChange={function (v) { onChange('ui', { ...ui, density: v }); }}
                        options={[
                            { value: 'compact', label: 'Compact' },
                ...
                <Row label="Button Style" description="Default button shape across the site">
                    <Select
                        value={ui.buttonStyle}
                        onChange={function (v) { onChange('ui', { ...ui, buttonStyle: v }); }}
                        options={[
                            { value: 'filled', label: 'Filled' },
                            { value: 'outline', label: 'Outline' },
                            { value: 'ghost', label: 'Ghost' },
                ...
                <Row label="Animations" description="Motion effects and transitions">
                    <Select
                        value={ui.animations}
                        onChange={function (v) { onChange('ui', { ...ui, animations: v }); }}
                        options={[
                            { value: 'full', label: 'Full Animations' },
...
function NotificationsTab({ settings, onChange }) {
    var notif = settings.notifications || DEFAULT_SETTINGS.notifications;

    return (
        <div className="space-y-6 animate-fade-in">
            <SectionHeader title="Notifications" description="Control notification preferences" />
            <div className="card p-4 space-y-0">
                <Row label="Enable Notifications" description="Turn all notifications on or off">
                    <Toggle id="notif-enabled" checked={notif.enabled} onChange={function (v) { onChange('notifications', { ...notif, enabled: v }); }} />
                </Row>
                <Row label="Sound Alerts" description="Play a sound on new notifications">
                    <Toggle id="notif-sound" checked={notif.sound} onChange={function (v) { onChange('notifications', { ...notif, sound: v }); }} />
                </Row>
                <Row label="Desktop Notifications" description="Show browser notifications">
                    <Toggle id="notif-desktop" checked={notif.desktopNotifications} onChange={function (v) { onChange('notifications', { ...notif, desktopNotifications: v }); }} />
                </Row>
                <Row label="Email Notifications" description="Get updates via email">
                    <Toggle id="notif-email" checked={notif.emailNotifications} onChange={function (v) { onChange('notifications', { ...notif, emailNotifications: v }); }} />
                </Row>
                <Row label="Push Notifications" description="Receive push notifications on your device">
                    <Toggle id="notif-push" checked={notif.pushNotifications} onChange={function (v) { onChange('notifications', { ...notif, pushNotifications: v }); }} />
                </Row>
                <Row label="Notification Frequency" description="How often to send notification digests">
                    <Select
                        value={notif.frequency}
                        onChange={function (v) { onChange('notifications', { ...notif, frequency: v }); }}
                        options={[
                            { value: 'instant', label: 'Instant' },
                            { value: 'daily', label: 'Daily Digest' },
                            { value: 'weekly', label: 'Weekly Summary' },
...
function AccessibilityTab({ settings, onChange }) {
    var a11y = settings.accessibility || DEFAULT_SETTINGS.accessibility;

    return (
        <div className="space-y-6 animate-fade-in">
            <SectionHeader title="Accessibility" description="Make the site usable for everyone" />
            <div className="card p-4 space-y-0">
                <Row label="Reduced Motion" description="Minimize animations for motion-sensitive users">
                    <Toggle id="a11y-motion" checked={a11y.reducedMotion} onChange={function (v) { onChange('accessibility', { ...a11y, reducedMotion: v }); }} />
                </Row>
                <Row label="High Contrast" description="Increase color contrast for better visibility">
                    <Toggle id="a11y-contrast" checked={a11y.highContrast} onChange={function (v) { onChange('accessibility', { ...a11y, highContrast: v }); }} />
                </Row>
                <Row label="Focus Indicators" description="Show visible keyboard focus outlines">
                    <Toggle id="a11y-focus" checked={a11y.focusVisible} onChange={function (v) { onChange('accessibility', { ...a11y, focusVisible: v }); }} />
                </Row>
                <Row label="Dyslexia-Friendly Font" description="Use OpenDyslexic font for readability">
                    <Toggle id="a11y-dyslexia" checked={a11y.dyslexiaFriendly} onChange={function (v) { onChange('accessibility', { ...a11y, dyslexiaFriendly: v }); }} />
                </Row>
                <Row label="Color Vision Mode" description="Adjust colors for color vision deficiencies">
                    <Select
                        value={a11y.colorVision}
                        onChange={function (v) { onChange('accessibility', { ...a11y, colorVision: v }); }}
                        options={[
                            { value: 'default', label: 'Default' },
                ...
                <Row label="Text Scale" description={'Scale: ' + a11y.textScale + 'x'}>
                    <input
                        type="range"
                        min="0.8" max="2" step="0.1"
                        value={a11y.textScale}
                        onChange={function (e) { onChange('accessibility', { ...a11y, textScale: parseFloat(e.target.value) }); }}
                        className="w-32 accent-accent"
                    />
                </Row>
            </div>
...
function DataTab({ settings, onChange }) {
    var data = settings.data || DEFAULT_SETTINGS.data;

    return (
        <div className="space-y-6 animate-fade-in">
            <SectionHeader title="Data & Storage" description="Manage auto-save, backups, and exports" />
            <div className="card p-4 space-y-0">
                <Row label="Auto-Save" description="Automatically save content changes">
                    <Toggle id="data-autosave" checked={data.autoSave} onChange={function (v) { onChange('data', { ...data, autoSave: v }); }} />
                </Row>
                <Row label="Save Interval" description="How often to auto-save (minutes)">
                    <div className="flex items-center gap-3">
                        <input
                            ...
                            min="1" max="60"
                            value={data.saveInterval}
                            onChange={function (e) { onChange('data', { ...data, saveInterval: parseInt(e.target.value) || 5 }); }}
                            className="input-base w-20 px-3 py-1.5 text-sm text-center"
                        />
                        <span className="text-sm text-muted">min</span>
                    </div>
                </Row>
                <Row label="Backup Enabled" description="Keep automatic backups of content">
                    <Toggle id="data-backup" checked={data.backupEnabled} onChange={function (v) { onChange('data', { ...data, backupEnabled: v }); }} />
                </Row>
                <Row label="Export Format" description="Default format for data exports">
                    <Select
                        value={data.exportFormat}
                        onChange={function (v) { onChange('data', { ...data, exportFormat: v }); }}
                        options={[
                            { value: 'json', label: 'JSON' },
                            { value: 'csv', label: 'CSV' },
                        ]}
...
function SiteTab({ settings, onChange }) {
    var site = settings.site || DEFAULT_SETTINGS.site;

    return (
        <div className="space-y-6 animate-fade-in">
            <SectionHeader title="Site Information" description="Global metadata displayed on the public site" />
            <div className="card p-4 space-y-4">
                <div>
                    ...
                    <input
                        type="text"
                        value={site.title || ''}
                        onChange={function (e) { onChange('site', { ...site, title: e.target.value }); }}
                        placeholder="My Awesome Site"
                ...
                <div>
                    ...
                    <textarea
                        rows={3}
                        value={site.description || ''}
                        onChange={function (e) { onChange('site', { ...site, description: e.target.value }); }}
                        placeholder="Describe your site for SEO..."
...
function ImportExportTab({ settings, onImport }) {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <SectionHeader title="Import / Export" description="Backup or restore your CMS configuration" />
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="card p-5">
                    ...
                    <p className="text-sm text-muted mb-4">Download your current settings as JSON.</p>
                    <button
                        onClick={handleExport}
                        className="w-full py-2 rounded-xl text-sm font-bold text-accent-foreground transition-all hover:opacity-90"
                ...
                <div className="card p-5">
                    ...
                    <textarea
                        ...
                        className="input-base w-full px-3 py-2 text-xs font-mono mb-3 resize-none"
                    />
                    {importStatus === 'success' && (
                    ...
                    {importStatus === 'error' && (
                        <div className="text-xs font-medium text-error mb-2">Invalid JSON format</div>
                    )}
                    <button
                        onClick={handleImport}
                        disabled={!importText.trim()}
                        className="w-full py-2 rounded-xl text-sm font-bold border border-accent text-accent transition-all hover:bg-accent/5 disabled:opacity-50"
                    >
                        Apply Imported Settings
                    </button>
                </div>
...
export default function Settings() {
    ...
    var [activeSubTab, setActiveSubTab] = useState('appearance');
    var [subOpen, setSubOpen] = useState(false); // mobile sub-nav

    useEffect(function () {
        fetch(API_BASE + '/settings')
            .then(function (r) { return r.json(); })
            .then(function (data) {
                setSettings(Object.keys(data).length > 0 ? { ...DEFAULT_SETTINGS, ...data } : DEFAULT_SETTINGS);
                setLoading(false);
            })
            .catch(function () {
                setSettings(DEFAULT_SETTINGS);
    ...
    var handleSave = function () {
        setSaving(true);
        var toSave = { ...settings, lastUpdated: new Date().toISOString() };
        fetch(API_BASE + '/settings', {
            method: 'POST',
    ...
    var handleImport = function (imported) {
        var merged = { ...DEFAULT_SETTINGS, ...imported, lastUpdated: new Date().toISOString() };
        setSettings(merged);
    };
        ...
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted mt-0.5">Configure site appearance, content, and behavior</p>
                </div>
                <button
                    onClick={handleSave}
                    disabled={saving}
                    className={[
                        'flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-accent-foreground transition-all',
                        saved ? 'bg-success' : '',
                    ].join(' ')}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                {/* Sub-Tab nav - mobile accordion toggle /}
                <div className="lg:col-span-3">
                    <button
                        ...
                        onClick={function () { setSubOpen(!subOpen); }}
                    >
                        <span>{subTabs.find(function (t) { return t.id === activeSubTab; })?.label}</span>
                        <Shield size={16} className="text-muted" />
                    </button>
                    <nav className={['space-y-1', subOpen ? '' : 'hidden lg:block'].join(' ')}>
                        {subTabs.map(function (tab) {
                            var Icon = tab.icon;
                            var isActive = activeSubTab === tab.id;
                            return (
                                <button
                                    key={tab.id}
                                    onClick={function () { setActiveSubTab(tab.id); setSubOpen(false); }}
                                    className={[
                ...
                <div className="lg:col-span-9">
                    ...
                    {activeSubTab === 'accessibility' && <AccessibilityTab settings={settings} onChange={handleChange} />}
                    {activeSubTab === 'data' && <DataTab settings={settings} onChange={handleChange} />}
                    {activeSubTab === 'importexport' && <ImportExportTab settings={settings} onImport={handleImport} />}
                </div>
            </div>
        </div>
    );

...
const API_BASE = '/api/admin/contact';
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="divide-y border rounded-xl overflow-hidden">
          {messages.map((msg) => (
            <div key={msg._id} className={`p-4 hover:bg-surface/50 ${msg.status === 'unread' ? 'bg-surface/30 border-l-4 border-l-accent' : ''}`}>
              <div className="flex justify-between items-start">
                ...
                <div className="flex gap-2 ml-4">
                  {msg.status === 'unread' && (
                    <button onClick={() => markAsRead(msg._id)} className="p-1.5 rounded hover:bg-surface" title="Mark as read">
                      <CheckCircle size={16} />
                    </button>
                  )}
                  <button onClick={() => deleteMessage(msg._id)} className="p-1.5 rounded hover:bg-red-100 text-red-500">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>

...
export default function ContentManager() {
  ...
  /*const handleSave = async () => {
    ...
    try {
      ...
      console.log(`Saving content - Size: ${payloadSize} bytes (${(payloadSize / 1024).toFixed(2)} KB)`);
...
export default function ContentManager() {
  ...
  const loadContent = async () => {
    ...
    try {
      ...
      if (data.data) {
        if (Array.isArray(data.data)) {
          homeContent = data.data.find(c => c.page === 'home');
        } else if (data.data.page === 'home') {
          homeContent = data.data;
        }
      ...
      if (homeContent && Object.keys(homeContent).length > 1) {
        // Only use the content from DB, don't merge with DEFAULT_CONTENT
        setContent(homeContent);
      } else {
        // Only use DEFAULT_CONTENT if no content exists in DB
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Only send the sections that have been modified
      // This creates a lean payload without default values
      const payload = {
        page: "home",
        tenantId: tenant,
        published: true,
        version: content.version || 1,
        updatedBy: "admin",
        // Only include sections that exist in the current content
        ...(content.navigation && { navigation: content.navigation }),
        ...(content.hero && { hero: content.hero }),
        ...(content.about && { about: content.about }),
        ...(content.areas && { areas: content.areas }),
      ...
      setSaved(true);
      // Dispatch event to notify frontend to refresh content
      window.dispatchEvent(new Event("content-updated"));
      localStorage.setItem('content_last_updated', Date.now().toString());
      setTimeout(() => setSaved(false), 2000);
    } catch (err) {
...
/*last stable version,just had a payload size error.inatuma everything back to db even when just changing 1 word
// src/components/ContentManager.jsx
import React, { useState, useEffect } from 'react';
import { Save, RefreshCw, Plus, Trash2, Check } from 'lucide-react';
...
export default function ContentManager() {
  ...
  const loadContent = async () => {
    ...
    try {
      ...
      if (Array.isArray(data.data)) {
        homeContent = data.data.find(c => c.page === 'home');
      } else if (data.data && data.data.page === 'home') {
        homeContent = data.data;
      }
      
      if (homeContent && Object.keys(homeContent).length > 1) {
        setContent({ ...DEFAULT_CONTENT, ...homeContent.data });
      } else {
        setContent({ ...DEFAULT_CONTENT });
      }
    } catch (err) {

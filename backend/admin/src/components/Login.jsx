...
export default function Login() {
  ...
  let tenantParam = new URLSearchParams(window.location.search).get('tenant');
  
  // If no tenant in URL, try to get from localStorage or default
  if (!tenantParam) {
    tenantParam = localStorage.getItem('default_tenant') || 'landscapes_integrity_solutions';
  }
...
const handleMasterGoogleLogin = () => {
  window.location.href = '/api/v1/auth/master/google';
    ...
    /* const handleGoogleLogin = () => {
        ...
        console.log('[Login] Starting Google auth with tenant:', tenantParam);
        window.location.href = `/api/v1/auth/google?tenant=${tenantParam || ''}`;
    };
        ...
        <div className="min-h-screen flex items-center justify-center p-4 bg-gray-100 dark:bg-gray-950">
            <div className="w-full max-w-md">
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-800 p-8">
                    ...
                    {/* Add this after the "Forgot Password?" button or before it /}
                    <div className="mt-4 text-center">
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                            Don't have an account?{' '}
                            <button
                            onClick={() => {
                                // Open registration modal - you'll need to add state for this
            ...
            <RegisterModal 
                ...
                onClose={() => setShowRegisterModal(false)}
                onSuccess={() => {
                    // Optional: Show success message or switch to login
